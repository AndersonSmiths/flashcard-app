import tkinter as tk
from tkinter import ttk
from tkinter import messagebox


class FlashcardApp():
    """
    A flashcard application that helps the user
    memorize terms and definitions via spaced repition

    Attributes
        root: a tkinter root
    """
    def __init__(self, root):
        self.root = root

        # creating main window 
        self.root.title("Flashcard App")
        self.root.geometry('1000x700')

        # create variables
        self.create_variables()

        # create notebook/tabs + buttons
        self.generate_notebook() # self initializes notebook
        self.set_tab()
        self.study_tab()


        # style/color
        #TODO

    def create_variables(self):
        self.set_name = tk.StringVar()
        self.term = tk.StringVar()
        self.definition = tk.StringVar()
        self.card_index = 0
        self.current_tabs = []


    def generate_notebook(self):
        self.notebook = ttk.Notebook(self.root)
        self.notebook.pack(fill='both', expand=True)
        
        # sets frame
        self.generate_set_frame = ttk.Frame(self.notebook)
        self.notebook.add(self.generate_set_frame, text='Create Set')

        # entry widgets
        ttk.Label(self.generate_set_frame, text='Set Name: ').pack(padx=5, pady=5)
        ttk.Label(self.generate_set_frame, textvariable=self.set_name, width=30).pack(padx=5, pady=5)

        ttk.Label(self.generate_set_frame, text='Term: ').pack(padx=5, pady=5)
        ttk.Label(self.generate_set_frame, textvariable=self.term, width=30).pack(padx=5, pady=5)

        ttk.Label(self.generate_set_frame, text='Definition: ').pack(padx=5, pady=5)
        ttk.Label(self.generate_set_frame, textvariable=self.definition, width=30).pack(padx=5, pady=5)

        # add word button (add_word)
        self.add_word_button = ttk.Button(self.generate_set_frame, text='Add Word').pack(padx=5, pady=10)

        # save set button (create_set)
        self.save_set_button = ttk.Button(self.generate_set_frame, text='Save Set').pack(padx=5, pady=10)
    
    def set_tab(self):
        self.select_set_frame = ttk.Frame(self.notebook)
        self.notebook.add(self.select_set_frame, text='Select Set')

        existing_sets = ttk.Combobox(self.select_set_frame, state='readonly')

        # select set button (select_set)
        self.select_set_button = ttk.Button(self.select_set_frame, text='Select Set').pack(padx=5, pady=5)

        # delete set button
        self.delete_set_button = ttk.Button(self.select_set_frame, text='Delete Set').pack(padx=5, pady=5)

    def study_tab(self):
        self.flashcards_frame = ttk.Frame(self.notebook)
        self.notebook.add(self.flashcards_frame, text='Study' )

        # display term  and definition on flashcard
        self.term_label = ttk.Label(self.flashcards_frame, text='', font=('TkDefaultFont', 24))
        self.term_label.pack(padx=5,pady=10)

        self.definition_label = ttk.Label(self.flashcards_frame, text='').pack(padx=5, pady=5)

        # flip flashcards (flip_card)
        self.flip_card_button = ttk.Button(self.flashcards_frame, text='Flip').pack(side='left',padx=5,pady=5)

        # button to view next card (next_card)
        self.next_card_button = ttk.Button(self.flashcards_frame, text='Next').pack(side='right', padx=5,pady=5)

        # button to view previous card previous_card
        self.previous_card_button = ttk.Button(self.flashcards_frame, text='Previous Card').pack(side='left',padx=5,pady=5)
